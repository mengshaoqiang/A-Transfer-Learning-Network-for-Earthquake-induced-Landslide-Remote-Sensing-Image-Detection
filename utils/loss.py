...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            #revise,(zhou)
            gain[2:6],shape = torch.tensor(p[i].shape)[[3, 2, 3, 2]],p[i].shape
            ...
            #indices.append((b, a, gj.clamp_(0, shape[3] - 1), gi.clamp_(0, shape[2] - 1))) 
            #revise(zhou,shape)
...
class ComputeLossAuxOTA:
    ...
    def find_3_positive(self, p, targets):
        ...
        for i in range(self.nl):
            anchors = self.anchors[i]
            #revise_error(zcs)
            #gain[2:6],shape = torch.tensor(p[i].shape)[[3, 2, 3, 2]],p[i].shape  # xyxy gain
            gain[2:6] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain
